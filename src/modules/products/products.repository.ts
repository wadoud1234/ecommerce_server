...
import { productsModel } from "src/infra/db/schema";
...
export class ProductsRepository implements IProductsRepository {
    ...
    async createProduct(dto: CreateProductDto): Promise<{ id: string, slug: string }> {
        const { name, price, quantity, storeId, categoryId, description, image } = dto
    ...
    async updateProduct(id: string, dto: UpdateProductDto): Promise<ProductEntity> {
        ...
        const product = await this.db.update(productsModel).set({
            name,
            slug: generateSlug(name),
            searchText: generateSearchText(name, description),
            price,
            ...
            description,
            image
        }).where(eq(productsModel.id, id)).returning()
        return product[0]
    }
